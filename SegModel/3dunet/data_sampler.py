...
from utils import *
import six
from augmentation_3D import Multi3DModalsGenerator
...
def get_train_dataflow(add_mask=True):
    ...
    def preprocess(data):
        if config.NO_CACHE:
            ...
            if config.MultiModalsGenerator:
                augment_generator = Multi3DModalsGenerator()
                volume_list, label, weight = augment_generator(np.array(volume_list), label, weight)
