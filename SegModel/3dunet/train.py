...
def offline_pred(pred_func, output_file):
        ...
        pred_brats(
            df, lambda img: segment_one_image(img, pred_func), output_file)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logdir', help='log directory', default='model/unet3d')
    parser.add_argument('--datadir', help='override config.BASEDIR')
    parser.add_argument('--step', help='model step', default='20000')
    ...
    if args.visualize or args.evaluate:
        ...
        if config.DYNAMIC_SHAPE_PRED:
            ...
            pred = OfflinePredictor(PredictConfig(
                    ...
                    session_init=get_model_loader("./model/unet-6-ax/model-" + args.step),
                    input_names=['image'],
                    output_names=get_model_output_names()))
            pred1 = OfflinePredictor(PredictConfig(
                    model=get_model(modelType="inference"),
                    session_init=get_model_loader("./model/unet-6-sa/model-" + args.step),
                    input_names=['image'],
                    output_names=get_model_output_names()))
            pred2 = OfflinePredictor(PredictConfig(
                    model=get_model(modelType="inference"),
                    session_init=get_model_loader("./model/unet-6-co/model-" + args.step),
                    input_names=['image'],
                    output_names=get_model_output_names()))
        ...
        if config.MULTI_VIEW:
            ...
            # autotune is too slow for inference
            os.environ['TF_CUDNN_USE_AUTOTUNE'] = '0'
            offline_pred([pred, pred1, pred2], args.outdir)
        else:
            pred = OfflinePredictor(PredictConfig(
                    model=get_model(modelType="inference"),
                    session_init=get_model_loader(args.load),
        ...
        cfg = TrainConfig(
            ...
            callbacks=[
                PeriodicCallback(
                    ModelSaver(max_to_keep=80, keep_checkpoint_every_n_hours=1),
                    every_k_epochs=1),
